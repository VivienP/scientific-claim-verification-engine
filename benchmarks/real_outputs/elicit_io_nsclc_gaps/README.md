...
> **Provisional verdicts — recomputed run pending.** The numbers in
> this README come from an archived baseline run that conflates three
> verdict classes (`unsupported`, `not_addressed`, and what is now
> `unverifiable`). Archived output at [`_archive_pre_fix/`](_archive_pre_fix/);
> recomputed run is pending. Replace this banner when it lands.
...
- `_archive_pre_fix/report.json` — archived prior pipeline output (25 claims, $0.64 cost). Does not reflect current pipeline behaviour; see banner above.
