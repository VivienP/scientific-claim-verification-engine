...
**First hand-labeled full-pipeline benchmark** ([`eval/e2e/`](eval/e2e/reference_paper_v1_results.md)): 7/25 (28%) verdict agreement on 25 cited claims from a domain-expert-annotated lactate ISF literature review (Perrelle 2023). This is up from the original 1/25 baseline and the interim 4/25 sprint result, but it is still below launch quality. Bibliography-aware citation routing, exact DOI lookup, PubMed PMID/title fallback, and a stricter verifier rubric improved DOI-labeled resolution to 17/20 and reduced `not_addressed` outputs from 23/25 to 10/25. The remaining failures are now mostly evidence-delivery and judgment failures: non-OA/abstract-only sources, multi-source citation ranges, and partial-support calibration. See [`eval/e2e/reference_paper_v1_results.md`](eval/e2e/reference_paper_v1_results.md) for the full diagnostic.
...
*Full-pipeline verdict agreement on the first hand-labeled benchmark: **7/25 (28%)** on 25 cited claims from Perrelle 2023 (lactate ISF literature review, domain-expert-annotated). Macro-F1 across 4 verdict classes: 0.28. The resolver is no longer the only story: the current run resolves 17/20 DOI-labeled claims to the audited DOI, but most remaining disagreements are blocked by abstract-only/non-OA evidence, multi-source citation ranges, or verifier calibration on `partially_supported`. The bibliography parser at [src/bibliography.py](src/bibliography.py), resolver changes in [src/resolve.py](src/resolve.py), PubMed fallback in [src/clients/pubmed.py](src/clients/pubmed.py), and verifier rubric update in [src/verify.py](src/verify.py) moved the system from 1/25 to 7/25. Confusion matrix, per-claim breakdown, and recovery analysis are in [`eval/e2e/reference_paper_v1_results.md`](eval/e2e/reference_paper_v1_results.md).*
