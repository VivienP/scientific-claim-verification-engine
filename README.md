...
| Real AI-for-science tools, 6 outputs, ~187 claims | full pipeline | **numbers being recomputed against the new schema** (Tracks A+D+F+G+I, 2026-05-12). Previous "67 supported / 29 partial / 31 unsupported / 60 not_addressed" headline was generated by a pre-fix pipeline that conflated `source contradicts` (unsupported) with `source is silent` and `pipeline could not access full text`. The post-fix pipeline distinguishes three distinct verdicts (`unsupported`, `not_addressed`, `unverifiable`) so the headline numbers reflect honest epistemic limits. | [benchmarks/real_outputs/](benchmarks/real_outputs/README.md) |
