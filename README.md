...
**Phase 5 generalization validation, 2026-05-08)**: pipeline run on the Valsci paper itself ([Edelman & Skolnick 2025, BMC Bioinformatics](https://doi.org/10.1186/s12859-025-06159-4)) — a different domain (bioinformatics), different citation style (numbered `[N]`), and the canonical paper of one of our closest related works. The first run exposed a bibliography-parser generalization bug (the regex only matched the LaTeX `[N]`-alone-on-line format from the lactate-ISF benchmark, silently failing on the BMC `1. Author` numbered format). After three iterative fixes (multi-format bibliography parser; citing-paper recursion guard; `ResolvedSourceSet.primary()` marker-order contract), the third run delivered **correct-source rate 10/11 = 90.9%** on externally-cited claims (oracle-verified via [`scripts/score_against_oracle.py`](scripts/score_against_oracle.py)), 2 multi-source aggregations fired, AAR scorecard **PCov 100% / PSnd 100% / CTran 47% / AEff 76.5 claims per USD**, total cost $0.25. Single residual defect (Wei CoT, no DOI in BMC bibliography) is delegated to a follow-up session via the prompt at [`reports/prompt_bug_b_arxiv_fallback.md`](reports/prompt_bug_b_arxiv_fallback.md). Investigation and resolution: [`reports/valsci_validation_investigation_2026-05-08.md`](reports/valsci_validation_investigation_2026-05-08.md). Per-claim breakdown: [`benchmarks/real_papers/valsci_brice_2025/README.md`](benchmarks/real_papers/valsci_brice_2025/README.md).
