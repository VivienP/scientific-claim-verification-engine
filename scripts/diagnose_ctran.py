...
``passages_searched_no_quote`` distinguishes "fulltext retrieved, BM25 selected passages,
LLM didn't quote any" from "no passages were ever shown to the LLM" (``no_evidence``).
The auditor still sees what was searched in the former case, so it counts as transparent.

A claim is a *failure* when ``source_passages`` is empty AND ``evidence_quality``
is outside that allow-list (typically ``no_evidence`` or ``citing_paper_context``).

This diagnoser categorises each failing claim into a fix-mode bucket so the
dominant failure mode is visible without guessing.
...
_TRANSPARENT_QUALITIES = frozenset(
    {
        ...
        "quoted_passage",
        "title_only",
        "passages_searched_no_quote",  # Fulltext + BM25 passages shown; LLM didn't quote.
    }
)
...
def _categorise(claim: dict[str, Any]) -> tuple[bool, str]:
    ...
    The transparent boolean mirrors :func:`src.aar._claim_is_transparent` so totals
    match the AAR scorecard. Failure categories sub-split by ``retrieval_status``
    so the dominant failure mode is identifiable from the diagnostic alone.
    """
...
def render_markdown(
    ...
    """
    lines: list[str] = []
    lines.append("# CTran failure diagnostic\n")
    lines.append(
...
def _recommendation_for(dominant_category: str) -> str:
    ...
    table = {
        "A1_doi_unresolved": (
            "→ Resolver / extractor scope. Fix the bibliography parser + CrossRef "
            "matcher before expecting retrieval changes to lift CTran."
        ),
