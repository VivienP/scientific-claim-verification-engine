...
- **Abstract-only numeric claims.** 5 of the 10 `unsupported` verdicts are claims about heart failure hospitalization, all-cause mortality, expanded MACE, or discontinuation rates from AMPLITUDE-O / EXSCEL secondary analyses where only the NEJM/Circulation abstract was accessible. The full text likely contains the specific numbers Elicit cites, but the abstract does not. The current pipeline correctly emits `unverifiable` with `unverifiable_reason="numeric_claim_abstract_only"` for these cases rather than a confident verdict. The numbers in this README are from the pre-fix archived run; see the banner at the top. A post-fix re-run is pending.
